...
class GCN_Encoder(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4, layer=2,device=None,use_ln=False,layer_norm_first=False):
        ...
        self.encodeModel = GCNConv(nfeat, nhid)
        self.decodeModel = GCNConv(nhid,nfeat)
        self.body = GCN_body(nfeat, nhid, dropout, layer,device=None,use_ln=use_ln,layer_norm_first=layer_norm_first)
        self.fc = nn.Linear(nhid,nfeat)
        self.outLayer = nn.Linear(nfeat,nclass)
    ...
    def decode(self,h):
        ...
        x = self.fc(h)
        return x
    def gcnDecode(self,x,edge_index,edge_weight=None):
        self.eval()
        x = self.decodeModel(x,edge_index,edge_weight)
        return x
